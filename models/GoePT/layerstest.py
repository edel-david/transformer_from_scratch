...
from .utilstest import compress_numpy_array, decompress_numpy_array
...
class Softmax:
    ...
    def backward(self, grad: ArrayLike):
        ...
        grad = (grad - (grad * self.output).sum(self.axis, keepdims=True)) * self.output
        return grad
...
class LayerNorm:
    ...
    def backward(self,grad_upstream:ArrayLike)->ArrayLike:
        ...
        dnorm = grad_upstream * self.weight
        grad_out = dnorm -dnorm.mean(self.axis,keepdims=True)-self.output * (dnorm * self.output).mean(self.axis,keepdims=True)
        return grad_out
